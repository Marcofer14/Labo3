...
def ingest_replay(
    ...
    *,
    ...
    existing_ids = load_indexed_ids(paths.index_path)
    ...
    return downloaded, parsed_written, new_samples
...
def rebuild_parsed(output_dir: Path, format_id: str, reparse: bool) -> None:
    ...
    for raw_path in raw_files:
        ...
        _, parsed_written, samples = ingest_replay(
            make_session(),
            battle_id=battle_id,
            format_id=format_id,
        ...
        print(f"{status}: {battle_id} (+{samples} samples)")
    print(f"Rebuild complete: {len(raw_files)} raw replays, {total_samples} new samples")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for battle_id, searched_user in deduped_targets:
        ...
        if format_id != args.format:
            ...
            continue
        try:
            downloaded, parsed_written, samples = ingest_replay(
                session,
                battle_id=battle_id,
                format_id=args.format,
                output_dir=output_dir,
        ...
        print(f"{raw_status}, {parsed_status}: {battle_id} (+{samples} samples)")
        if args.sleep > 0:
            time.sleep(args.sleep)
